def generate_exam_html(items, output_filename, title, header_text, items_per_row, font_size):
    """
    Generates a generic exam HTML file.
    """
    html_content = [
        "<!DOCTYPE html>",
        "<html>",
...
def format_study_sheet_html(characters_data, output_filename):
    """
    Generates an HTML file for a study sheet.

    Args:
        characters_data: A list of tuples, where each tuple contains
                         (character, pinyin, word).
        output_filename: The name of the output HTML file (without extension).
    """
    html = """
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        th, td {
            ...
            padding: 15px;
            text-align: center;
            height: 60px; /* Increased height for writing space */
        }
        th {
...
def format_find_words_html(words, sentence, grid, start_row, output_filename, title="找朋友"):
    """
    Generates an HTML file for a 'find words' game, mimicking the format in the image.
    """
    # Calculate the top position for the start marker.
    # Each cell is 1/8th of the grid height. We want the middle of the cell.
    # (100% / 16) gets us to the middle of the first cell.
    # (100% / 8) * start_row moves it down by the number of rows.
...
<body>
    ...
    <div class="container">
        <ul class="word-list">
            {''.join(f'<li class="word-item">{word}</li>' for word in words)}
        </ul>
        ...
        <div class="grid-container">
            <div class="char-grid">
                {''.join(f'<div class="grid-cell">{char if char is not None else ""}</div>' for row in grid for char in row)}
            </div>

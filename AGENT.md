...
`scripts/run_test_server.sh` can be used to run a Django server locally for testing. It will create a virtual environment, install dependencies, and start the server.

The server will be running locally at 127.0.0.1:8000. You can use `curl` command to issue GET and POST requests to test the server functionality. You can review the server log at src/django.log to find any errors. Before you issue another request, you can use `> src/django.log` command to truncate the log.

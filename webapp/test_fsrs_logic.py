...
class TestFsrsLogic(unittest.TestCase):
    ...
    def test_build_cards(self):
        ...
        all_records = [
            {'id': 1, 'date': '2025-08-23', 'character': '在', 'type': 'read', 'score': 10},
            {'id': 2, 'date': '2025-08-23', 'character': '说', 'type': 'write', 'score': 10},
            {'id': 3, 'date': '2025-08-23', 'character': '九', 'type': 'read', 'score': 10},
            {'id': 51, 'date': '2025-08-23', 'character': '九', 'type': 'write', 'score': 10},
        ]

        # Run the card building logic
        built_cards = fsrs_logic.build_cards(unique_characters, all_records)
        review_date = datetime.datetime.strptime('2025-08-23', '%Y-%m-%d').replace(tzinfo=datetime.timezone.utc)

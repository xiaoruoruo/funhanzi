...
class Selection:
        ...
        for (char, card_type_), card in fsrs_logic.cards.items():
            if card_type_ == card_type:
                ...
                if card_type == 'read':
                    ...
                    retrievability = fsrs_logic.write_scheduler.get_card_retrievability(card, today)
                
                if retrievability is not None:
                    card_data = {'char': char, 'retrievability': float(retrievability), 'card': card}
    ...
    def from_failed_records(self, record_type: str, cutoff_date: str, threshold: int) -> 'Selection':
        """
    ...
    def remove_recent_records_by_type(self, days: int, types: List[str]) -> 'Selection':
        ...
        """
        if not types:
            raise ValueError("types list cannot be empty when calling remove_recent_records_by_type")
        return self._remove_records(days, types)

    def _remove_records(self, days: int, types: Optional[List[str]] = None) -> 'Selection':
        """
        ...
        if self.is_fsrs_mode:
            ...
            self.fsrs_cards = [card for card in self.fsrs_cards if card['char'] not in recent_chars]
        else:
            # Filter the character list
            self.characters = [char for char in self.characters if char not in recent_chars]
    ...
    def remove_score_greater(self, record_type: str, score: float) -> 'Selection':
        ...
        if self.is_fsrs_mode:
            ...
            self.fsrs_cards = [card for card in self.fsrs_cards if scores.get(card['char'], 0) <= score]
        else:
            # Filter character list based on scores
            self.characters = [char for char in self.characters if scores.get(char, 0) <= score]
    ...
    def lowest_retrievability(self) -> 'Selection':
        ...
        if not self.is_fsrs_mode:
            raise ValueError("lowest_retrievability() can only be called after from_fsrs()")
    ...
    def retrievability(self, min_val: float = -1, max_val: float = 1) -> 'Selection':
        ...
        self.fsrs_cards = [card for card in self.fsrs_cards 
